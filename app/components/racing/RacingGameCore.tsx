...
interface RacingGameCoreProps {
  ...
  onUpdateStats: (newStats: Partial<RacingGameCoreProps['gameStats']>) => void;
  onGameEnd: () => void;
  onTogglePause: () => void;
  onQuitGame: () => void;
}

export function RacingGameCore({
  gameState,
  gameStats,
  onUpdateStats,
  onGameEnd,
  ...
  useEffect(() => {
    ...
    function calcMovement() {
      ...
      totalDistance += $.state.speed * 0.1;
      const currentScore = Math.floor(totalDistance * ($.state.speed / 10));
      ...
      onUpdateStats({
        ...
        score: currentScore,
        maxSpeed: Math.max(gameStats.maxSpeed, Math.floor($.state.speed))
      });

      // End game after 2 minutes or if speed is 0 for too long
      const gameTime = Date.now() - gameStartTime;
      if (gameTime > 120000 || ($.state.speed === 0 && gameTime > 10000)) { // 2 min or no speed for 10s
        gameRunning = false;
    ...
    // Game controls based on gameState
  ...
  }, [gameState, onUpdateStats, onGameEnd, gameStats.maxSpeed]);
    ...
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex-1 flex items-center justify-center p-2">
        <canvas 
          ...
          width={420}
          height={650}
          className="border border-[var(--app-card-border)] rounded-lg shadow-xl max-w-full max-h-full"
          style={{ aspectRatio: '420/650' }}
        />
        ...
        {gameState === 'paused' && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center rounded-lg">
            <div className="text-center text-white">
              <h2 className="text-xl font-bold mb-4">⏸️ PAUSED</h2>
              <Button
                variant="primary"
      ...
      <div className="p-4 bg-[var(--app-card-bg)] border-t border-[var(--app-card-border)]">
        ...
        <div className="mt-2 text-center text-xs text-[var(--app-foreground-muted)]">
          <p>🎮 Use ← → ↑ ↓ arrow keys to race</p>
        </div>
      </div>
    </div>
  );
